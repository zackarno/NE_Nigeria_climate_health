...
**Note:** to add more data to compiled rds:
    
- add code to create data in same format (ie. ADM_NAME, value, parameter, date, level, time_desc)
- bind_rows with mdf loaded in `combine_to_monthly` chunk.
- re-write rds after checking
- proceed to `write_to_shiny` chunk
...
# C:\Users\zack.arno\AppData\Local\r-miniconda\envs\rgee/python.exe in future sessions.
# reticulate::py_install(packages = 'earthengine-api', envname = 'C:/Users/zack.arno/AppData/Local/r-miniconda/envs/rgee')

# trick go to tools, project options (or global options), python
ee_Initialize(drive = T)
# ee_Initialize(user = 'ndef', drive = TRUE)
# ee_install_upgrade()
# ee_install_set_pyenv("C:/Users/zack.arno/AppData/Local/r-miniconda/envs/rgee")
# 16 day ndvi composites
ndvi_dat_src <- "MODIS/006/MOD13Q1"
ndvi <- ee$ImageCollection(ndvi_dat_src)

LST <-  ee$ImageCollection("MODIS/006/MOD11A2")$select('LST_Day_1km')
...
LST_gte2016 <-  LST$filterDate("2016-01-01", "2022-01-01")
...
# LST 16 day
system.time(
  LST_gte2016_agg_mean <-spatial_levels |> 
  map2(.y = spatial_level_extract_cols,.f = ~ee_extract_long(ic = LST_gte2016,
                                                             sf = .x,
                                                             sf_col = .y,
                                                             scale =1000,
                                                             reducer = "mean")
...
# viirs$first()$projection()$nominalScale()$getInfo()
# VIIRS monthly
system.time(
  VIIRS_gte2016_agg_mean <-spatial_levels |> 
  map2(.y = spatial_level_extract_cols,.f = ~ee_extract_long(ic = viirs,
                                                             sf = .x,
                                                             sf_col = .y,
                                                             scale =500,
                                                             reducer = "mean")
...
# LST
 LST_gte2016_agg_mean_df <- names(LST_gte2016_agg_mean) |> 
  map_dfr(~LST_gte2016_agg_mean[[.x]] |> 
        mutate(
          level = .x,
          time_desc = "8-day MODIS" 
        ) |> 
        rename_with(.fn = ~"ADM_NAME",.cols = matches("ADM"))
        ) #|> 
  
# VIIRs
 VIIRS_gte2016_agg_mean_df <- names(VIIRS_gte2016_agg_mean) |> 
  map_dfr(~VIIRS_gte2016_agg_mean[[.x]] |> 
        mutate(
          level = .x,
          time_desc = "1 month VIIRS" 
        ) |> 
        rename_with(.fn = ~"ADM_NAME",.cols = matches("ADM"))
        ) #|> 
...
LST_2000_2010<- LST$filterDate("2000-01-01", "2010-12-31")$
  select("NDVI","EVI")
...
vci<- ndvi_gte2016_w_historical_bands$map(
  function(img){
    vci<- img$expression(
...
vci |> 
  ee_print()
# simoplify expression procedure into functions
########################################
# bex <- c("NDVI","NDVI_max","NDVI_min")
# calc_vars_split <- stringr::str_split(
#   "float((NDVI-NDVI_min)/(NDVI_max-NDVI_min))",pattern = "\\*|-|\\+|^\\/|\\(|\\)"
# )[[1]]
# 
# library(tidyverse)
# bla <- calc_vars_split[calc_vars_split%in%bex]
# bla |> 
#   map(
# ~list(
#   function(im) im$select(.x)
# )
# 
# ) |> set_names(bla)
# 
...
#   print(blat)
#   bla_list[[i]] <- list(
#      function(x){x$select(eval(blat))}
#   )
#   
# }
# function(ic, expression){
#   ic$map(
#     function(img){
#       img_calc <-  img$expression(
#         
#       )
#     }
#   )
...
# ck <- ee_extract_long(ic = ndvi_zscore,
#                 sf = spatial_levels$mj_lgas_dissolved,
#                 sf_col = "ADM2_EN",
#                 scale =250,
#                 reducer = "mean")
# 
...
# 
# ck2<- ee_extract_long(ic = vci,
#                 sf = spatial_levels$mj_lgas_dissolved,
#                 sf_col = "ADM2_EN",
#                 scale =250,
#                 reducer = "mean")
  # VCI
# debugonce(ee_extract_long)
system.time(
  vci_gte2016_agg_mean <-spatial_levels |> 
  map2(.y = spatial_level_extract_cols,.f = ~ee_extract_long(ic = vci,
                                                             sf = .x,
                                                             sf_col = .y,
                                                             scale =250,
                                                             reducer = "mean")
...
vci_gte2016_agg_mean_df <- names(vci_gte2016_agg_mean) |> 
  map_dfr(~vci_gte2016_agg_mean[[.x]] |> 
            filter(parameter=="VCI") |> 
        mutate(
          level = .x,
          time_desc = "16-day MODIS" 
        ) |> 
...
# i think i should take the average vci per month
...
copy above chunk, but for LST
```{r}



LST_2000_2010<- LST$filterDate("2000-01-01", "2010-12-31")
# LST_2000_2010 |> ee_get_date_ic()


# I need to figure out trick to combine reducers so the 4 stats below can be calculated together
 LST_10yr_mean<- yr_mo_composite_stat_ic2(dat =LST_2000_2010,
                                      month_range = c(1,12),
                                      year_range = c(2000,2010),
                                      stat = "mean",
                                      monthly_stat_per = "range") |>
 ...
 LST_10yr_sd<- yr_mo_composite_stat_ic2(dat =LST_2000_2010,
                                      month_range = c(1,12),
                                      year_range = c(2000,2010),
                                      stat = c("sd"),
                                      monthly_stat_per = "range") |> 
   ic_add_month_property()
 LST_10yr_min<- yr_mo_composite_stat_ic2(dat =LST_2000_2010,
                                      month_range = c(1,12),
                                      year_range = c(2000,2010),
                                      stat = c("min"),
                                      monthly_stat_per = "range") |> 
 ...
 LST_10yr_max<- yr_mo_composite_stat_ic2(dat =LST_2000_2010,
                                      month_range = c(1,12),
                                      year_range = c(2000,2010),
                                      stat = c("max"),
                                      monthly_stat_per = "range") |> 
 ...
 # new func
 LST_10yr_stats <- LST_10yr_mean |> 
   ic_join_bands(y = LST_10yr_sd,by = "moy") |>  
   ic_join_bands(y=LST_10yr_max,by = "moy") |> 
   ic_join_bands(y=LST_10yr_min,by = "moy") 



LST_gte2016_w_historical_bands<- ic_join_bands(x = LST_gte2016 |> ic_add_month_property(),
                                                    y = LST_10yr_stats |> ic_add_month_property(),
                                                    by = "moy")

LST_gte2016_w_historical_bands$first()$bandNames()$getInfo()
# this is still a bit ugly - opened issue wrap simplifier
LST_zscore<- LST_gte2016_w_historical_bands$map(
  function(img){
    zscore<- img$expression(
      "float((LST_Day_1km-LST_Day_1km_mean)/(LST_Day_1km_stdDev))",
      opt_map= list(LST_Day_1km= img$select("LST_Day_1km"),
                    LST_Day_1km_mean= img$select("LST_Day_1km_mean"),
                    LST_Day_1km_stdDev= img$select("LST_Day_1km_stdDev")
      )
    )$rename("LST_z_score")
    img$select("LST_Day_1km")$addBands(zscore)
  }
...
# ck <- ee_extract_long(ic = ndvi_zscore,sf = spatial_levels$mj_lgas_dissolved,sf_col = "ADM2_EN",scale = 250,reducer = "mean")
  ...
  # LST
system.time(
  LST_zscore <-spatial_levels |> 
  map2(.y = spatial_level_extract_cols,.f = ~ee_extract_long(ic = LST_zscore,
                                                             sf = .x,
                                                             sf_col = .y,
                                                             scale =1000,
                                                             reducer = "mean")
...
#
LST_zscore_gte2016_agg_mean_df <- names(LST_zscore) |> 
  map_dfr(~LST_zscore[[.x]] |> 
        mutate(
          level = .x,
          time_desc = "8-day MODIS" 
        ) |> 
...
modis_chirps_smap_awd_otp_mz_time_raw_df <- bind_rows(modis_chirps_smap_awd_otp_time_raw_df |> 
  ...
  # 
# modis_chirps_smap_awd_otp_mz_time_raw_df |> 
  # write_rds(here::here("data/modis_
...
# put this isn her 2/6/222 - ont sure if it belongs
modis_chirps_smap_awd_otp_mz_time_raw_df <- read_rds(here::here("data/modis_chirps_smap_awd_otp_orig_ts.rds"))

admins$lga |> 
  filter(str_detect(ADM2_EN,"^G")) |> print(n=30)
admins <-  load_nga_admins()
pop <-  read_csv(here::here("data/pop_lga_msna2019_sf.csv"))
pop |> 
  left_join( admins$lga) |> 
  arrange(ADM2_EN) |> 
  filter(str_detect(ADM2_EN,"^G"))|> print(n=30)
otp_monthly_full |> 
  filter(str_detect(ADM_NAME,"^G"))
...
otp_cleaned |> 
  filter(!is.na(muac_total_screened))
...
otp_cleaned|> 
  ...
  summarise(
    ...
    sites_reporting = sum(!is.na(new_otp_admissions)),
    .groups = "drop"
  ) |> 
  ggplot(aes(x= sites_reporting,y=otp_cases))+
  geom_point()
...
otp_by_lga <-
  ...
  summarise(
    otp_cases=sum(new_otp_admissions,na.rm=T),
    sites_reporting = sum(!is.na(new_otp_admissions)),
    otp_cases_per_site = otp_cases/sites_reporting,
    .groups = "drop_last"
  ) |> 
  mutate(median_sites_reporting= median(sites_reporting,na.rm=T)) |> 
  ungroup() |> 
  left_join(
    admins$lga |> 
      st_drop_geometry() |> 
  ...
  rename(ADM_NAME="LGA") |> 
  select(ADM_NAME,date, otp_cases,otp_cases_per_1000,sites_reporting,otp_cases_per_site) |> 
  pivot_longer(otp_cases:otp_cases_per_site,names_to = "parameter") |> 
  mutate(
    level= "ne_lgas_all",
...
mdf <- bind_rows(mdf |> 
  filter(time_desc!="1-month OTP") ,
otp_by_lga)

# mdf |> count(parameter)
# mdf |> write_rds(here::here("data/modis_chirps_smap_awd_otp_orig_ts2.rds"))
```
...
- any additional RS aggregates can be calculated aove and added in here as doe in the bind_rows ~line 

```{r combine_to_monthly}
library(janitor)
library(lubridate)
library(glue)
...
mdf <- read_rds(here::here("data/modis_chirps_smap_awd_otp_orig_ts2.rds"))

# mdf <- bind_rows(mdf,VIIRS_gte2016_agg_mean_df)
# mdf <- mdf |> write_rds(here::here("data/modis_chirps_smap_awd_otp_orig_ts2.rds"))

mdf |> count(parameter)
mdf |> count(level)

mdf |> filter(year(date)>=2017) |> filter(ADM_NAME =="Kukawa") |> 
   mutate(
    mo_date= glue("{year(date)}-{month(date)}-01"),
    level2= if_else(level %in% c("ne_lgas_all","ne_lgas_borno","lga"),"lga",level)
  ) |> 
  filter(level2=="lga",parameter=="otp_cases_per_1000")
# since i calculated vci above i can take that out put nd put it here
# acled_monthly_full
# mdf2<- bind_rows(mdf,acled_monthly_full)


# mdf2 |> write_rds(here::here("data/modis_chirps_smap_awd_otp_orig_ts.rds"))
mdf |> 
  filter(time_desc=="1-month OTP")
...
monthly_lga_level <- lga_level_dat |> 
  ...
  mutate(value= if_else(
    parameter%in%c("precipitation","awd_cases","otp_cases"),
    value_sum,value_mean))# will result in average # of ward reporting... 
...
otp_monthly_full |> select(VCI)


# otp_monthly_full |> 
#   write_rds(here::here("data/modis_chirps_smap_awd_otp_orig_ts2.rds"))
```
...
acled_monthly <- acled_ne_simp |> 
  st_drop_geometry() |> 
  group_by(
    ADM2_EN,
    mo_date=ymd(glue("{year(date)}-{month(date)}-01"))
  ) |> 
  summarise(
    acled_fatalities = sum(fatalities),
    acled_events= n(),.groups = "drop"
  ) |> 
  pivot_longer(acled_fatalities:acled_events) |> 
  mutate(
    level = "lga",
    time_desc = "ACLED"
...
acled_monthly %>% 
  group_by(ADM_NAME,date) %>% 
  complete(value) %>% 
  ungroup()

acled_monthly_full <- complete(acled_monthly, ADM_NAME, parameter,date,fill = list(value=0, level="lga",time_desc="ACLED"))


```

write it out to shiny app directory
```{r write_to_shiny}
admins <- load_nga_admins()
...
otp_monthly_full |>
  left_join(admins$lga, by=c("ADM_NAME"="ADM2_EN")  ) |> 
  saveRDS(file = here::here("../impact-initiatives-geospatial/nga_climate_health_data_explorer/data/otp_monthly_ex4.rds"))
# otp_monthly_full |>
#   left_join(admins$lga, by=c("ADM_NAME"="ADM2_EN")  ) |> 
#   saveRDS(file = here::here("data/otp_monthly_ex4.rds"))

read_rds( file = here::here("../impact-initiatives-geospatial/nga_climate_health_data_explorer/otp_monthly_ex1.rds"))
read_rds( file = here::here("../impact-initiatives-geospatial/nga_climate_health_data_explorer/data/adm2_nga.rds"))
# admins$lga |>
#   filter(ADM1_EN %in% c("Borno","Adamawa","Yobe")) |>
#   write_rds(here::here("../impact-initiatives-geospatial/nga_climate_health_data_explorer/adm2_nga.rds"))
  
# dir(here::here("../impact-initiatives-geospatial/nga_climate_health_data_explorer/"))
...
nga_admins <- load_nga_admins()  
nga_admins_lga <- nga_admins$lga
nga_model_vis <- function(df=nest_otp,lm_formula){
  ...
  model_stats_tbl <- model_stats_tbl |> 
    mutate(
      sig_tf= if_else(p.value<0.05,"green","orange")
    )
      
  sig_admin_names <- model_stats_tbl |> 
    filter(p.value<0.05) |> pull(ADM_NAME)
    
   
   dependent= all.vars(formulaized_lm)[1]
   independents= all.vars(formulaized_lm)[-1]
   ...
   plot_list<- independents |> 
       map(~sig_df_unnested |> 
       ggplot(aes(x=!!sym(.x),
                  y= !!sym(dependents) ))+
         labs(x=.x, y= dependents)+
         geom_point()+
         geom_smooth(method="lm",formula="y~x")+
         facet_wrap(~ADM_NAME,scales = "free_y")
       ) |> 
     set_names(paste0("plot_",independents))
   
   # create leaflet map
  ne_lga_admins <- nga_admins_lga |> 
    filter(ADM1_EN %in% c("Borno","Adamawa","Yobe"))
   ne_nga_admin_vals<- ne_lga_admins |> 
     left_join(
        model_stats_tbl |> 
     select(ADM_NAME,r.squared, adj.r.squared, p.value,sig_tf),
     by=c("ADM2_EN"="ADM_NAME")
       
     )
  
  
   bins <- c(0,0.1,0.2,0.25,0.3,0.33,0.37,0.4,0.44)
   pal <- leaflet::colorBin("YlGn", domain = abs(ne_nga_admin_vals$adj.r.squared), bins = bins)
   
   tooltip <- paste(
    "State: ", ne_nga_admin_vals$ADM1_EN,"<br/>", 
    "LGA: ", ne_nga_admin_vals$ADM2_EN, "<br/>", 
    "adjusted r squared ", round(ne_nga_admin_vals$adj.r.squared, 2),"<br/>", 
    "p value ", round(ne_nga_admin_vals$p.value, 3), "<br/>", 
    sep="") %>%
  lapply(htmltools::HTML)

   res$leaflet_map <- leaflet::leaflet(ne_nga_admin_vals) |> 
     leaflet::addProviderTiles(provider =leaflet::providers$Esri.WorldImagery ) |> 
     # leaflet::addTiles(leaflet::providers$Esri.WorldImagery) |> 
     leaflet::addPolygons(
       color=ne_nga_admin_vals$sig_tf,
       fillColor = ~pal(abs(adj.r.squared)),
       label= tooltip,fillOpacity = 1,group = "admin"
   ) |> 
     leaflet::addLayersControl(
       baseGroups = c("OSM (default)", "Toner", "Toner Lite"),
       overlayGroups = c("admin"),
       options = leaflet::layersControlOptions(collapsed = FALSE)
  )
   list(plot_list,res) |> flatten()
...
otp_monthly_full |> arrange(ADM_NAME)
  
mod_res<- nga_model_vis(
  df = nest_otp,
  lm_formula = "otp_cases_per_1000~NDVI_z_score")  

windows();mod_res$plot_NDVI_z_score
windows();mod_res$plot_NDVI_z_score
mod_res$plot_NDVI_z_score
mod_res$leaflet_map
